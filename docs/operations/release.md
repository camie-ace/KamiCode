...
2. Create an explicit App ID for `tech.camie.kamicode` and enable Associated Domains.
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.tech.camie.kamicode` and includes
    Associated Domains.
