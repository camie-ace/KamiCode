...
An empty `Claude HOME path` means KamiCode uses your normal home directory.
...
KamiCode only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so KamiCode keeps separate Claude homes isolated instead of
...
2. Add or configure a Claude provider in KamiCode.
