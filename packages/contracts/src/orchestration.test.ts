...
it.effect("accepts non-image chat attachment metadata", () =>
  Effect.gen(function* () {
    ...
    if (video.type !== "video" || !("durationMs" in video)) {
      assert.fail("Expected a video attachment with duration metadata.");
    }
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(parsed.scheduledFor, null);
    }),
);

it.effect("decodes thread.turn-start-requested scheduling metadata", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeThreadTurnStartRequestedPayload({
      threadId: "thread-1",
      messageId: "msg-1",
    ...
    assert.strictEqual(parsed.scheduledFor, "2026-01-02T00:00:00.000Z");
  }),
);
