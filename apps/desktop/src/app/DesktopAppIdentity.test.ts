      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(input.legacyPathExists === true && path.includes("KamiCode (Alpha)")),
            readFileString: () =>
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["KamiCode (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "KamiCode (Alpha)");
