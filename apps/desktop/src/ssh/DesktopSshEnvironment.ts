...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "KamiCode window is not available for SSH authentication.";
      break;
...
const makePasswordPrompt = (
  ...
  request: (() => {
    ...
    return (request: SshAuth.SshPasswordRequest) => {
      ...
      return prompts.request(request).pipe(Effect.mapError(toSshPasswordPromptError));
    };
  })(),
});
...
export const make = Effect.gen(function* () {
  ...
  return DesktopSshEnvironment.of({
    ...
    ensureEnvironment: (target, ensureOptions) => {
      const ensurePasswordPrompt = SshAuth.SshPasswordPrompt.of(
      ...
      return manager
        ...
        .pipe(
          Effect.provideService(SshAuth.SshPasswordPrompt, ensurePasswordPrompt),
          Effect.provide(runtimeContext),
