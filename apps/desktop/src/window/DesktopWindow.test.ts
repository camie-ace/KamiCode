      ...
      Layer.mergeAll(
        input.iconPaths
          ? Layer.succeed(DesktopAssets.DesktopAssets, {
              iconPaths: Effect.succeed(input.iconPaths),
              resolveResourcePath: () => Effect.succeed(Option.none<string>()),
            })
          : desktopAssetsLayer,
        input.environmentInput
          ? DesktopEnvironment.layer(input.environmentInput).pipe(
              Layer.provide(
                Layer.mergeAll(
                  NodeServices.layer,
                  DesktopConfig.layerTest({
...
describe("DesktopWindow", () => {
  ...
  it.effect("applies the Windows window icon from desktop resources", () =>
    Effect.gen(function* () {
      ...
      const mainWindow = yield* Ref.make<Option.Option<Electron.BrowserWindow>>(Option.none());
      const createdWindowOptions: Electron.BrowserWindowConstructorOptions[] = [];
      const iconPath = "C:\\Program Files\\KamiCode\\resources\\icon.ico";
      const layer = makeTestLayer({
        window: fakeWindow.window,
        createCount,
        mainWindow,
      ...
      yield* Effect.gen(function* () {
        ...
        assert.equal(createdWindowOptions[0]?.icon, iconPath);
