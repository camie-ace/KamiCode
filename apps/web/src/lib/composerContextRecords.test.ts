...
describe("attachment context records", () => {
  it("keeps a native video linked to its persisted upload through the wire schema", () => {
    const record = attachmentContextRecord({
      attachment: {
        type: "video",
        id: "clip-1",
        name: "review.mp4",
        ...
        file: new File(["clip"], "review.mp4", { type: "video/mp4" }),
      },
      attachmentId: "persisted-video",
    });
    const context = decodeMessageContext({ version: 1, records: [record] });
    const resolved = resolveUserMessageContext({
      text: "[review.mp4](t3-context://v1/file/file_clip-1)",
      context,
    });
    expect(resolved.recordsById.get("file_clip-1")).toMatchObject({
      kind: "file",
      attachmentId: "persisted-video",
      mimeType: "video/mp4",
    });
