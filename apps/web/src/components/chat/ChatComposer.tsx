...
import { isChatQueueShortcut, isChatScheduleShortcut } from "../../keybindings";
import {
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerAttachments = attachmentDraft.attachments;
  ...
  const nonPersistedComposerAttachmentIds = attachmentDraft.nonPersistedAttachmentIds;
  ...
  const attachmentProcessingKey = attachmentTargetKey;
  ...
  const composerSendState = useMemo(
    () =>
      deriveComposerSendState({
        prompt,
        imageCount: composerAttachments.length + composerFiles.length,
        terminalContexts: composerTerminalContexts,
  ...
  const nonPersistedComposerImageIdSet = nonPersistedComposerAttachmentIdSet;
  const visibleComposerAttachments = useMemo(() => {
    ...
    return composerAttachments.filter(
      (attachment) => attachment.type !== "image" && !previewAnnotationIds.has(attachment.id),
    );
  ...
  const addComposerAttachmentsToDraft = useCallback(
    (attachments: ComposerAttachment[]) => {
      return addComposerDraftAttachments(attachmentDraftTarget, attachments);
    },
    [attachmentDraftTarget, addComposerDraftAttachments],
  );
  ...
  const removeComposerAttachmentFromDraft = useCallback(
    (attachmentId: string) => {
      ...
      removeComposerDraftAttachment(attachmentDraftTarget, attachmentId);
    },
    [
      attachmentDraftTarget,
      questionAttachmentTarget,
  ...
  const scheduleShortcutLabel = useMemo(
    () =>
      shortcutLabelForCommand(keybindings, "chat.schedule", {
        context: { terminalOpen, terminalFocus: false },
      }),
    [keybindings, terminalOpen],
  );
  ...
  const openScheduleDialog = useCallback(() => {
    if (!composerSendState.hasSendableContent || activePendingProgress) return;
  ...
  const onComposerCommandKey = (
    ...
    if (
      ...
      !activePendingProgress &&
      composerSendState.hasSendableContent &&
      isChatScheduleShortcut(event, keybindings, {
        context: { terminalOpen, terminalFocus: false },
      })
    ) {
      openScheduleDialog();
  ...
  const stashCurrentPrompt = useCallback(async () => {
    ...
    const nonImageAttachments = composerAttachmentsRef.current.filter(
      (attachment) => attachment.type !== "image",
    );
    if (nonImageAttachments.length > 0) {
      toastManager.add({
        type: "error",
        title: "Remove non-image attachments before stashing",
        description: "The prompt stash currently preserves text, images, and uploaded files.",
      });
      return;
    }
  ...
  const countReservedAttachments = () => {
    ...
    return (
      ...
      composerFilesRef.current.length +
      (pendingAttachmentProcessingRef.current.get(attachmentTargetKey) ?? 0) +
  ...
  const addComposerAttachments = async (
    ...
    if (options?.source?._tag === "pasted-text") {
      ...
      const availableSlots =
        ...
        composerAttachmentsRef.current.length -
        composerFilesRef.current.length -
        (pendingAttachmentProcessingRef.current.get(attachmentProcessingKey) ?? 0);
      for (const file of files.slice(0, Math.max(0, availableSlots))) {
        if (fileStagingLimit === null || file.size <= 0 || file.size > fileStagingLimit) {
          toastManager.add({
            type: "error",
            title:
              fileStagingLimit === null
                ? "This server does not support file attachments."
                : fileAttachmentTooLargeMessage(file.name, fileStagingLimit),
          });
          continue;
        }
        stagedFiles.push({
          ...
          id: randomUUID(),
          name: file.name || "file",
          mimeType: inferComposerFileMimeType(file),
          sizeBytes: file.size,
          file,
          source: options.source,
        });
      }
      const acceptedIds = new Set(addComposerFilesToDraft(stagedFiles));
      const accepted = stagedFiles.filter((file) => acceptedIds.has(file.id));
      if (accepted.length === 0) return false;
      const inserted = insertAttachmentReferences(
        accepted.map(fileContextReference),
        options.selection,
      );
      const attached = accepted[0]!;
      toastManager.add({
        type: "info",
        title: `Large paste attached as ${attached.name}`,
        description: `${formatAttachmentSize(attached.sizeBytes)} · Use ${isMacPlatform(navigator.platform) ? "⌘⇧V" : "Ctrl+Shift+V"} to keep a large paste inline.`,
        data: { hideCopyButton: true },
      });
      return inserted;
    }
    ...
    // switch threads or questions while a large image is being compressed.
    ...
    if (questionAttachmentTarget)
      changeQuestionAttachmentPreparation(questionAttachmentTarget, acceptedFiles.length);
    try {
  ...
  const removeComposerAttachment = (imageId: string) => {
    ...
    void confirmation.then((confirmed) => {
      if (confirmed) removeComposerAttachmentFromDraft(imageId);
    });
  };
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      hasPendingAttachments: () =>
        (pendingAttachmentProcessingRef.current.get(attachmentTargetKey) ?? 0) > 0,
                ...
                <ComposerContextActionsContext value={composerContextActions}>
                  <ComposerPromptEditor
                    ...
                    disabled={
                      ...
                      isChoiceOnlyPendingQuestion ||
                      activePendingIsResponding ||
                      (!activePendingProgress && noProviderAvailable)
                    }
                  />
                </ComposerContextActionsContext>
