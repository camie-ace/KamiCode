...
function workEntryHasPinnedRichContent(entry: WorkLogEntry): boolean {
  return (
    entry.agentSpawn !== undefined ||
    entry.evidenceRun !== undefined ||
    entry.projectTrigger !== undefined
  );
}
      ...
      if (visibleGroupedEntries.length > 0) {
        const onlyToolEntries = visibleGroupedEntries.every(
          (entry) =>
            ...
            !workEntryHasPinnedRichContent(entry) &&
            entry.tone !== "error",
        );
        ...
        if (onlyToolEntries && activeInProgressToolEntries.length > 0) {
          ...
          const groupId = workGroupId(timelineEntry.id, timelineEntry.entry);
          const expanded = input.expandedWorkGroupIds?.has(groupId) ?? false;
          // Rich CTA/card rows are always visible: active fleets, evidence,
          // and trigger results must never hide behind a "+N tool calls"
          // toggle. Selection is by membership (pinned OR recent-tail),
          // preserving the group's chronological order in both collapsed and
          // expanded states (review finding: concatenating two filtered lists
          // moved a mid-group pinned row above earlier tool rows).
          const overflowCandidates = visibleGroupedEntries.filter(
            (entry) => !workEntryHasPinnedRichContent(entry),
          );
          ...
          const visibleEntries = visibleGroupedEntries.filter(
            (entry) => workEntryHasPinnedRichContent(entry) || !hiddenIds.has(entry.id),
          );
