...
): attachment is ChatImageAttachment | ChatGifAttachment {
...
function UserVideoAttachment({
  ...
  readonly file: ChatFileAttachment | ChatVideoAttachment;
}) {
...
function UserTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  const userImages = userAttachments.filter(isTimelineImageAttachment);
  const userFiles = userAttachments.filter(isFileAttachment);
  const userVideos = userAttachments.filter(
    (attachment): attachment is ChatFileAttachment | ChatVideoAttachment =>
      attachment.type === "video" ||
      (isFileAttachment(attachment) && isVideoAttachment(attachment)),
  );
  const otherUserFiles = userFiles.filter((file) => !isVideoAttachment(file));
  const unknownAttachments = userAttachments.filter(
    (attachment) =>
      !isTimelineImageAttachment(attachment) &&
        ...
        {(regularImages.length > 0 || userVideos.length > 0) && (
          <div className="mb-2 grid max-w-[420px] grid-cols-2 gap-2">
            {regularImages.map((image) => (
              <div
                key={image.id}
                data-attachment-kind={image.type}
...
function UserMessagePreviewAnnotationCard(props: {
  ...
  image: ChatImageAttachment | ChatGifAttachment | null;
}) {
  const ctx = use(TimelineRowCtx);
  return (
...
function EvidenceArtifactLink(props: {
  label: string;
...
function EvidenceRunCard({ evidenceRun }: { evidenceRun: EvidenceRunWorkEntry }) {
  ...
  const hiddenScreenshotCount = Math.max(
    0,
    evidenceRun.screenshots.length - visibleScreenshots.length,
  );
  const summary =
  ...
  return (
    <div className="mt-2 overflow-hidden rounded-xl border border-border/70 bg-background/55">
      <div className="flex flex-wrap items-start justify-between gap-2 border-b border-border/55 px-3 py-2">
        <div className="min-w-0">
      ...
      <div className="space-y-3 px-3 py-3">
        <p className="whitespace-pre-wrap wrap-break-word text-xs leading-5 text-foreground/82">
          {summary}
        </p>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {visibleScreenshots.map((screenshot) => (
                <a
                  key={`${evidenceRun.runId}:screenshot:${screenshot.path}`}
        ...
        {evidenceRun.videos.length > 0 ? (
          <div>
            ...
            <div className="space-y-2">
              {evidenceRun.videos.map((video) => (
                <div
                  key={`${evidenceRun.runId}:video:${video.path}`}
