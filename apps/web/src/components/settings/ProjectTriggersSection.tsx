...
export default function ProjectTriggersSection({
  ...
  const submitForm = useCallback(
    ...
    [editingTrigger, form, reloadTriggers, selectedProject],
  );
  ...
  const toggleTriggerEnabled = useCallback(
    ...
    [pendingAction, reloadTriggers, selectedExecutionLocation, selectedProject],
  );
  ...
  const runTrigger = useCallback(
    ...
    [pendingAction, reloadTriggers, selectedExecutionLocationLabel, selectedProject],
  );
  ...
  }, [deleteTarget, pendingAction, reloadTriggers, selectedProject]);
    ...
    <>
      ...
      >
        {projectOptions.length === 0 ? (
          ...
          <SettingsRow
            title="Project"
            description={selectedProject?.cwd ?? "Select a loaded project."}
            status={
              selectedProject
        ...
        {loadState.status === "loaded" && triggers.length === 0 ? (
          <SettingsRow
            title="No triggers"
            description={`No schedules are configured for ${selectedProject?.name ?? "this project"}.`}
            control={
