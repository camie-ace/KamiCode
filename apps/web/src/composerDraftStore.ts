    ...
    (setBase, get) => {
      ...
      return {
        ...
        addAttachment: (threadRef, attachment) => {
          ...
          const alreadyAdded =
            existing?.attachments.some(({ id }) => id === attachment.id) === true ||
            existing?.images.some(({ id }) => id === attachment.id) === true;
          get().addAttachments(
          ...
          return (
            !alreadyAdded &&
            (get().draftsByThreadKey[threadKey]?.attachments.some(
              ({ id }) => id === attachment.id,
            ) ??
              false)
          );
        ...
        addImage: (threadRef, image) => {
          return get().addAttachment(threadRef, image);
        },
        ...
        removeImage: (threadRef, imageId) => {
          get().removeAttachment(threadRef, imageId);
        },
        addFiles: (threadRef, files) => {
          ...
          set((state) => {
            ...
            if (accepted.length === 0 && replacements.size === 0) return state;
