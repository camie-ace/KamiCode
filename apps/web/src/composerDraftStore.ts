...
export interface ComposerVideoAttachment extends Omit<ChatVideoAttachment, "previewUrl"> {
  previewUrl: string;
  file: File;
  status?: "ready";
}
...
/** A hydrated draft file whose upload did not finish must be reattached. */
...
interface ComposerDraftStoreState {
  ...
  addAttachment: (threadRef: ComposerThreadTarget, attachment: ComposerAttachment) => boolean;
  addAttachments: (
    threadRef: ComposerThreadTarget,
    attachments: ComposerAttachment[],
  ...
  /** Move prompt/images when an unsent draft changes project. */
...
function composerImageDedupKey(image: ComposerImageAttachment): string {
  return composerAttachmentDedupKey(image);
}
...
function verifyPersistedAttachments(
  ...
  set((state) => {
    ...
    const existingNonImageNonPersistedIds = current.nonPersistedAttachmentIds.filter(
      (id) => !imageIdSet.has(id),
    );
    ...
    const nextDraft: ComposerThreadDraftState = {
      ...current,
      persistedAttachments,
      nonPersistedAttachmentIds: [...existingNonImageNonPersistedIds, ...nonPersistedImageIds],
      nonPersistedImageIds,
    };
...
function toHydratedThreadDraft(
  ...
  const images = hydrateImagesFromPersisted(persistedDraft.attachments);
  return {
    ...
    (setBase, get) => {
      ...
      return {
        ...
        addAttachment: (threadRef, attachment) => {
          ...
          const acceptedIds = get().addAttachments(
            typeof threadRef === "string" ? DraftId.make(threadKey) : threadRef,
            [attachment],
          );
          return acceptedIds.includes(attachment.id);
        },
        addAttachments: (threadRef, attachments, options) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          ...
          set((state) => {
            ...
            for (const attachment of attachments) {
              ...
              if (
                ...
                if (attachment.previewUrl && !acceptedPreviewUrls.has(attachment.previewUrl)) {
                  revokeObjectPreviewUrl(attachment.previewUrl);
                }
                continue;
              }
              if (
                existing.attachments.length + existing.files.length + dedupedIncoming.length >=
                PROVIDER_SEND_TURN_MAX_ATTACHMENTS
              ) {
                if (attachment.previewUrl && !acceptedPreviewUrls.has(attachment.previewUrl)) {
                  revokeObjectPreviewUrl(attachment.previewUrl);
            ...
            const dedupedImages = dedupedIncoming.filter(isComposerImageAttachment);
            const nonPersistedAttachmentIds = new Set(existing.nonPersistedAttachmentIds);
            for (const attachment of dedupedIncoming) {
              if (!isComposerImageAttachment(attachment)) {
                nonPersistedAttachmentIds.add(attachment.id);
              }
            }
            ...
            return {
              draftsByThreadKey: {
                ...
                [threadKey]: {
                  ...
                  images: [...existing.images, ...dedupedImages],
                  nonPersistedAttachmentIds: Array.from(nonPersistedAttachmentIds),
                },
              },
            };
          });
        ...
        removeAttachment: (threadRef, attachmentId) => {
          ...
          const removedAttachment =
            existing.attachments.find((attachment) => attachment.id === attachmentId) ??
            existing.images.find((image) => image.id === attachmentId);
          if (removedAttachment?.previewUrl) {
            revokeObjectPreviewUrl(removedAttachment.previewUrl);
          }
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current) {
            ...
            const nextDraft: ComposerThreadDraftState = {
              ...
              images: current.images.filter((image) => image.id !== attachmentId),
              ...
              prompt: removeInlineContextReference(
                current.prompt,
                toKindScopedComposerContextId("image", attachmentId),
              ).prompt,
        ...
        addImage: (threadRef, image) => get().addAttachment(threadRef, image),
        addImages: (threadRef, images, options) => get().addAttachments(threadRef, images, options),
        removeImage: (threadRef, imageId) => get().removeAttachment(threadRef, imageId),
        addFiles: (threadRef, files, options) => {
          ...
          set((state) => {
            ...
            for (const file of files) {
              ...
              if (
                existing.attachments.length + existing.files.length + accepted.length >=
                PROVIDER_SEND_TURN_MAX_ATTACHMENTS
              ) {
                break;
              }
        ...
        removeFile: (threadRef, fileId) => {
          ...
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current?.files.some((file) => file.id === fileId)) return state;
            const nextDraft = {
              ...current,
              prompt: removeInlineContextReference(
                current.prompt,
          ...
          set((state) => {
            ...
            const movedPrompt = ensureInlineContextReferences(
              [
                ...
                ...source.reviewComments.map(reviewCommentContextReference),
                ...source.previewAnnotations.map(previewAnnotationContextReference),
                ...source.files.map(fileContextReference),
              ].reduce(
                (text, reference) => removeInlineContextReference(text, reference.contextId).prompt,
                source.prompt,
              ),
              [
                ...destination.terminalContexts.map(terminalContextReference),
                ...destination.reviewComments.map(reviewCommentContextReference),
