...
const make = Effect.gen(function* () {
  ...
  const toSecretSettingsError = (operation: ServerSettingsOperation, cause: unknown) =>
    new ServerSettingsError({
      settingsPath,
      operation,
      cause,
    ...
    updateSettings: (patch) =>
      writeSemaphore.withPermits(1)(
        Effect.gen(function* () {
          ...
          const nextPersisted = yield* persistHostedCollaborationToken(persisted.settings).pipe(
          ...
          const materialized = yield* Effect.uninterruptibleMask(() =>
            Effect.gen(function* () {
              ...
              const materializedExit = yield* Effect.exit(
                materializeProviderEnvironmentSecrets(next).pipe(
                  Effect.flatMap(materializeHostedCollaborationToken),
                  Effect.flatMap(materializeHostedBrowserProxyUrl),
                  Effect.flatMap(materializeSharedCollaborationProfileTokens),
                ),
