...
const makeProjectionThreadRepository = Effect.gen(function* () {
    ...
    execute: (row) =>
      sql`
        ...
        VALUES (
          ...
          ${row.branch},
          ${row.worktreePath},
          ${row.startedBy === undefined || row.startedBy === null ? null : JSON.stringify(row.startedBy)},
          ${row.workflowParentThreadId ?? null},
          ${row.workflowLaneId ?? null},
          ${row.workflowLaneRole ?? null},
        ...
        DO UPDATE SET
          ...
          started_by_json = excluded.started_by_json,
          workflow_parent_thread_id = excluded.workflow_parent_thread_id,
          workflow_lane_id = excluded.workflow_lane_id,
          workflow_lane_role = excluded.workflow_lane_role,
  ...
  const getById: ProjectionThreadRepositoryShape["getById"] = (input) =>
    getProjectionThreadRow(input).pipe(
      Effect.map(Option.map((row) => ({ ...row, locked: row.locked === 1 }))),
      Effect.mapError(toPersistenceSqlError("ProjectionThreadRepository.getById:query")),
    );

  const listByProjectId: ProjectionThreadRepositoryShape["listByProjectId"] = (input) =>
    listProjectionThreadRows(input).pipe(
      Effect.map((rows) => rows.map((row) => ({ ...row, locked: row.locked === 1 }))),
