...
const make = Effect.gen(function* () {
  ...
  const resumeTurnsAfterCompaction = Effect.fn("resumeTurnsAfterCompaction")(function* (
    ...
    while (queued.length > 0 && turnsAfterCompaction.get(threadId) === queued) {
      ...
      const { messageId, scheduledFor, ...request } = event.payload;
  ...
  const ensureSessionForThread = Effect.fn("ensureSessionForThread")(function* (
    ...
    if (existingSessionThreadId) {
      ...
      const resumeCursor =
        shouldRestartForModelChange || (instanceChanged && !providerResumeStateCompatible)
          ? undefined
  ...
  const drainThreadQueue = Effect.fn("drainThreadQueue")(function* (threadId: ThreadId) {
    ...
    try {
      while (true) {
        ...
        const latestThread = yield* resolveThreadDetail(threadId);
        ...
        const beforeSendThread = yield* resolveThreadDetail(threadId);
