...
describe("ProviderCommandReactor", () => {
      ...
      const session: ProviderSession = {
        ...
        interactionMode:
          ...
          typeof input.interactionMode === "string"
            ? (input.interactionMode as ProviderSession["interactionMode"])
            : DEFAULT_PROVIDER_INTERACTION_MODE,
        ...(typeof input === "object" &&
        input !== null &&
        "cwd" in input &&
  ...
  it("drains a queued turn after the active session settles", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";

    await harness.dispatch({
      type: "thread.session.set",
      commandId: CommandId.make("cmd-session-set-queue-drain-running"),
      threadId: ThreadId.make("thread-1"),
      session: {
        threadId: ThreadId.make("thread-1"),
        status: "running",
        ...
        runtimeMode: "approval-required",
        activeTurnId: asTurnId("turn-queue-drain-active"),
        lastError: null,
        updatedAt: now,
      },
      createdAt: now,
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-queue-drain-2"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-queue-drain-2"),
        role: "user",
        text: "second",
        attachments: [],
      },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-set-queue-drain-stopped"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "stopped",
  ...
  it("holds a scheduled turn until it is due, then drains it through the queue", async () => {
    const harness = await createHarness();
    const now = await Effect.runPromise(DateTime.now);
    const scheduledFor = DateTime.formatIso(DateTime.add(now, { milliseconds: 750 }));

    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-scheduled"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-scheduled"),
        role: "user",
        text: "send when due",
        attachments: [],
      },
  ...
  it("keeps a due scheduled turn queued until the active turn settles", async () => {
    const harness = await createHarness();
    const now = DateTime.formatIso(await Effect.runPromise(DateTime.now));

    await harness.dispatch({
      type: "thread.session.set",
      commandId: CommandId.make("cmd-session-running-before-schedule"),
      threadId: ThreadId.make("thread-1"),
      session: {
        threadId: ThreadId.make("thread-1"),
        status: "running",
        ...
        providerInstanceId: ProviderInstanceId.make("codex"),
        runtimeMode: "approval-required",
        activeTurnId: asTurnId("turn-active-before-schedule"),
        lastError: null,
        updatedAt: now,
      },
      createdAt: now,
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-scheduled-while-running"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-scheduled-while-running"),
        role: "user",
        text: "send after the active turn",
        attachments: [],
      },
      interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
      runtimeMode: "approval-required",
      dispatchPolicy: "queue",
      scheduledFor: DateTime.formatIso(
        DateTime.add(DateTime.makeUnsafe(now), { milliseconds: 300 }),
      ),
      createdAt: now,
    });
    ...
    const settledAt = DateTime.formatIso(await Effect.runPromise(DateTime.now));
    await harness.dispatch({
      type: "thread.session.set",
      commandId: CommandId.make("cmd-session-stopped-after-schedule"),
      threadId: ThreadId.make("thread-1"),
      session: {
        threadId: ThreadId.make("thread-1"),
        status: "stopped",
    ...
    await waitFor(() => harness.sendTurn.mock.calls.length === 1);
    expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
      input: "send after the active turn",
    });
  });

  it("lets an ordinary queued turn run past an earlier future schedule", async () => {
    const harness = await createHarness();

    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-future-scheduled"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-future-scheduled"),
        role: "user",
        text: "future scheduled",
        attachments: [],
      },
      interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
      runtimeMode: "approval-required",
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-ordinary-queued"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-ordinary-queued"),
        role: "user",
    ...
    await Effect.runPromise(Effect.sleep("100 millis"));
    await waitFor(() => harness.sendTurn.mock.calls.length === 1);
    expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({ input: "ordinary queued" });
    await Effect.runPromise(Effect.sleep("100 millis"));
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
  ...
  it("restarts an existing Codex thread on a same-provider instance with fresh resume state", async () => {
    const harness = await createHarness({
      continuationKeyByInstance: {
        codex: "codex:home:/default",
        codex_backup: "codex:home:/backup",
      },
    });
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-fresh-codex-1"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-fresh-codex-1"),
          role: "user",
          text: "first",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-fresh-codex-2"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-fresh-codex-2"),
          role: "user",
          text: "second",
          attachments: [],
        },
    ...
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.session?.providerInstanceId).toBe(ProviderInstanceId.make("codex_backup"));
    expect(
  ...
  it("restarts the provider session when interaction mode is updated on the thread", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-interaction-mode-1"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-interaction-mode-1"),
          role: "user",
          text: "first",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.interaction-mode.set",
        commandId: CommandId.make("cmd-interaction-mode-set-trigger"),
        threadId: ThreadId.make("thread-1"),
    ...
    await waitFor(async () => {
      const readModel = await harness.readModel();
      const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
      return thread?.interactionMode === "trigger";
    });

    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-interaction-mode-2"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-interaction-mode-2"),
          role: "user",
          text: "second",
          attachments: [],
        },
  ...
  it("allows a server-generated Waterfall retry to cross provider drivers", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-set-stopped-waterfall"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "stopped",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("provider:usage-event:waterfall-turn-start:retry-id"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-waterfall-provider-switch"),
          role: "user",
          text: "continue through the waterfall",
          attachments: [],
        },
        modelSelection: {
          instanceId: ProviderInstanceId.make("claudeAgent"),
    ...
    expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
      input: expect.stringContaining("Current user request:\ncontinue through the waterfall"),
      modelSelection: {
        instanceId: ProviderInstanceId.make("claudeAgent"),
        model: "claude-opus-4-6",
      },
