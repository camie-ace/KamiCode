...
const ProjectionProjectDbRowSchema = ProjectionProject.mapFields(
  Struct.assign({
    ...
    testEnvironments: Schema.fromJsonString(Schema.Array(ProjectTestEnvironment)),
  }),
);
...
const ProjectionThreadDbRowSchema = ProjectionThread.mapFields(
  Struct.assign({
    ...
    startedBy: Schema.NullOr(Schema.fromJsonString(ThreadStartedBy)),
  }),
);
...
const REQUIRED_SNAPSHOT_PROJECTORS = [
  ...
  ORCHESTRATION_PROJECTOR_NAMES.turnQueue,
...
function mapQueuedTurn(
  ...
  return {
    queueId: row.queueId as OrchestrationQueuedTurn["queueId"],
    ...
    completedAt: row.completedAt,
    turnId: row.turnId,
    failureDetail: row.failureDetail as OrchestrationQueuedTurn["failureDetail"],
  };
}

function countQueuedTurnsByThread(
  rows: ReadonlyArray<Schema.Schema.Type<typeof ProjectionQueuedTurnDbRowSchema>>,
): Map<string, number> {
...
function mapProjectShellRow(
  ...
  return {
    ...
    scripts: row.scripts,
    testEnvironments: row.testEnvironments,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
    ...
    execute: () =>
      sql`
        SELECT
          ...
          test_environments_json AS "testEnvironments",
    ...
    execute: () =>
      sql`
        SELECT
          ...
          started_by_json AS "startedBy",
          workflow_parent_thread_id AS "workflowParentThreadId",
          workflow_lane_id AS "workflowLaneId",
          workflow_lane_role AS "workflowLaneRole",
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              for (const row of queuedTurnRows) {
                ...
                threadQueuedTurns.push(mapQueuedTurn(row));
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                workflowParentThreadId: row.workflowParentThreadId ?? null,
                workflowLaneId: row.workflowLaneId ?? null,
                workflowLaneRole: row.workflowLaneRole ?? null,
  ...
  const getShellSnapshot: ProjectionSnapshotQueryShape["getShellSnapshot"] = () =>
    sql
      .withTransaction(
        Effect.all([
          ...
          listQueuedTurnRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getShellSnapshot:listQueuedTurns:query",
                "ProjectionSnapshotQuery.getShellSnapshot:listQueuedTurns:decodeRows",
              ),
            ),
          ),
          listProjectionStateRows(undefined).pipe(
      ...
      .pipe(
        Effect.flatMap(
          ([projectRows, threadRows, sessionRows, latestTurnRows, queuedTurnRows, stateRows]) =>
            Effect.gen(function* () {
              let updatedAt: string | null = null;
              for (const row of projectRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
              ...
              const queuedTurnCountByThread = countQueuedTurnsByThread(queuedTurnRows);
      ...
      .withTransaction(
        Effect.all([
          ...
          listQueuedTurnRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getArchivedShellSnapshot:listQueuedTurns:query",
                "ProjectionSnapshotQuery.getArchivedShellSnapshot:listQueuedTurns:decodeRows",
              ),
            ),
          ),
          listProjectionStateRows(undefined).pipe(
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    testEnvironments: option.value.testEnvironments,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        worktreePath: threadRow.value.worktreePath,
        startedBy: threadRow.value.startedBy ?? null,
        workflowParentThreadId: threadRow.value.workflowParentThreadId ?? null,
        workflowLaneId: threadRow.value.workflowLaneId ?? null,
        workflowLaneRole: threadRow.value.workflowLaneRole ?? null,
        ...
        queuedTurnCount: countQueuedTurnsByThread(queuedTurnRows).get(threadId) ?? 0,
      } satisfies OrchestrationThreadShell);
    });
  ...
  const getThreadDetailById: ProjectionSnapshotQueryShape["getThreadDetailById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      const thread = {
        ...
        session: Option.isSome(sessionRow) ? mapSessionRow(sessionRow.value) : null,
        queuedTurns: queuedTurnRows.map(mapQueuedTurn),
      };
