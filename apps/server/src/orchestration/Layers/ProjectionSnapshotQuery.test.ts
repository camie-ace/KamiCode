...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  it.effect("hydrates read model from projection tables and computes snapshot sequence", () =>
    Effect.gen(function* () {
      ...
      // A merged link plus a newer open one. The multi-link projection keeps
      // both; without a resolvable project repository identity, the legacy
      // one-slot projection safely retains the persisted JSON fallback below.
      yield* sql`
      ...
      // Without link rows the multi-link projection is empty, while the legacy
      // one-slot projection still exposes its persisted compatibility fallback.
      yield* sql`DELETE FROM projection_thread_pull_requests`;
      ...
      if (unlinkedShell._tag === "Some") {
        ...
        assert.deepEqual(unlinkedShell.value.linkedPullRequest, {
          projectId: asProjectId("project-1"),
          repository: "pingdotgg/t3code",
          number: 41,
          url: "https://github.com/pingdotgg/t3code/pull/41",
