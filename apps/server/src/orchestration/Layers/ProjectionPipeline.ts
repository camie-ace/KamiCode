...
export const ORCHESTRATION_PROJECTOR_NAMES = {
  ...
  turnQueue: "projection.turn-queue",
...
const makeOrchestrationProjectionPipeline = Effect.fn("makeOrchestrationProjectionPipeline")(
  function* () {
      ...
      switch (event.type) {
        case "thread.created":
          ...
          yield* projectionThreadRepository.upsert({
            ...
            startedBy: event.payload.startedBy ?? null,
            workflowParentThreadId: event.payload.workflowParentThreadId ?? null,
            workflowLaneId: event.payload.workflowLaneId ?? null,
            workflowLaneRole: event.payload.workflowLaneRole ?? null,
        ...
        case "thread.meta-updated": {
          ...
          yield* projectionThreadRepository.upsert({
            ...
            ...(event.payload.modelSelection !== undefined
              ? { modelSelection: event.payload.modelSelection }
              : {}),
            ...(event.payload.locked !== undefined ? { locked: event.payload.locked } : {}),
    ...
    const applyTurnQueueProjection: ProjectorDefinition["apply"] = Effect.fn(
      "applyTurnQueueProjection",
    )(function* (event, _attachmentSideEffects) {
      switch (event.type) {
        case "thread.turn-start-requested": {
          if (event.payload.dispatchPolicy !== "queue") {
    ...
    const projectors: ReadonlyArray<ProjectorDefinition> = [
      ...
      {
        name: ORCHESTRATION_PROJECTOR_NAMES.turnQueue,
        apply: applyTurnQueueProjection,
      },
      {
