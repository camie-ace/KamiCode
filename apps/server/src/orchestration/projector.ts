  ...
  switch (event.type) {
      ...
      return decodeForEvent(ProjectCreatedPayload, event.payload, event.type, "payload").pipe(
        Effect.map((payload) => {
          ...
          const nextProject = {
            ...
            testEnvironments: payload.testEnvironments ?? [],
          ...
          projects: nextBase.projects.map((project) =>
            project.id === payload.projectId
              ? {
                  ...
                  ...(payload.testEnvironments !== undefined
                    ? { testEnvironments: payload.testEnvironments }
                    : {}),
                  updatedAt: payload.updatedAt,
                }
      ...
      return Effect.gen(function* () {
        ...
        const thread: OrchestrationThread = yield* decodeForEvent(
          OrchestrationThread,
          {
            ...
            workflowParentThreadId: payload.workflowParentThreadId ?? null,
            workflowLaneId: payload.workflowLaneId ?? null,
            workflowLaneRole: payload.workflowLaneRole ?? null,
            latestTurn: null,
            queuedTurns: [],
            createdAt: payload.createdAt,
            updatedAt: payload.updatedAt,
    ...
    case "thread.turn-start-requested":
      return Effect.gen(function* () {
        ...
        if (payload.dispatchPolicy !== "queue") {
          return nextBase;
        }
        const thread = nextBase.threads.find((entry) => entry.id === payload.threadId);
        if (!thread) {
        ...
        return {
          ...
          threads: updateThread(nextBase.threads, payload.threadId, {
            ...
            queuedTurns: (thread.queuedTurns ?? []).filter(
              (turn) => turn.queueId !== payload.queueId,
            ),
            updatedAt: event.occurredAt,
          }),
        };
