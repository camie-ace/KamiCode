...
> {
  switch (command.type) {
    ...
    case "thread.turn.start": {
      ...
      if (command.scheduledFor !== undefined && dispatchPolicy !== "queue") {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: "Scheduled turns must use the queue dispatch policy.",
        });
      }
      if (command.scheduledFor !== undefined) {
        ...
        if (!Number.isFinite(scheduledForMs)) {
          return yield* new OrchestrationCommandInvariantError({
            commandType: command.type,
            detail: "Scheduled turn time must be a valid instant.",
          });
        }
      }
      if (
      ...
      const turnEvents = [...lifecycleResetEvents, userMessageEvent, turnStartRequestedEvent];
