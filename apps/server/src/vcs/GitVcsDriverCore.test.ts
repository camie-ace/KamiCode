...
const isWorktreePathScan = (args: ReadonlyArray<string>) =>
  args.includes("for-each-ref") && args.some((arg) => arg.includes("%(worktreepath)"));
    ...
    Effect.gen(function* () {
      ...
      const countingSpawner = ChildProcessSpawner.make((command) =>
        Effect.gen(function* () {
          ...
          const isWorktreeScan = isWorktreePathScan(command.args);
          const shouldDelay =
      ...
      const worktreeScans = firstSnapshotCommands.filter(isWorktreePathScan);
    ...
    Effect.gen(function* () {
      ...
      const incompleteMetadataSpawner = ChildProcessSpawner.make((command) =>
        Effect.gen(function* () {
          ...
          const isWorktreeList = isWorktreePathScan(command.args);
    ...
    Effect.gen(function* () {
      ...
      const staleWorktreeSpawner = ChildProcessSpawner.make((command) =>
        Effect.gen(function* () {
          ...
          const isWorktreeList = isWorktreePathScan(command.args);
          if (isWorktreeList) {
            return makeSuccessfulHandle(`refs/heads/stale-worktree\0${missingWorktreePath}\0\n`);
          }
          return yield* delegate.spawn(command);
        }),
      );
