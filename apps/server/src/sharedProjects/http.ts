...
} from "../userAuth/Services/UserAuth.ts";
...
const projectIdFromQuery = Effect.gen(function* () {
  ...
  const projectId = Option.isSome(url)
    ? url.value.searchParams.get("projectId")?.trim()
    : null;
  if (!projectId) {
...
const SHARED_COLLAB_PROFILE_HEADER =
  "x-kamicode-shared-collaboration-profile-id";

type HostedCollabQuery = Record<
  string,
...
const hostedCollabConfig = Effect.gen(function* () {
  ...
  const requestedProfileId =
    request.headers.get(SHARED_COLLAB_PROFILE_HEADER)?.trim() ?? "";
  ...
  if (settings.sharedCollaborationDefaultTarget.mode === "profile") {
    const defaultProfile =
      settings.sharedCollaborationProfiles.find(
        (candidate) =>
          candidate.id === settings.sharedCollaborationDefaultTarget.profileId,
      ) ?? null;
    if (defaultProfile) {
      ...
      return yield* hostedCollabConfigFromUrlAndToken({
        ...
        token: profileToken,
        invalidUrlMessage:
          "Default shared collaboration profile URL is not valid.",
      });
    }
  }
...
const decodeHostedPayloadJson = Schema.decodeEffect(
...
const withSharedProjectErrorHandling = <R>(
  effect: Effect.Effect<
    HttpServerResponseModule.HttpServerResponse,
    SharedProjectsRouteError,
    R
  >,
...
export const sharedProjectsBootstrapRouteLayer = HttpRouter.add(
  ...
  withSharedProjectErrorHandling(
    Effect.gen(function* () {
      ...
      const projectId = yield* projectIdFromQuery;
      const hosted =
        yield* hostedSharedProjectRequest<SharedProjectBootstrapManifest>({
          user,
          method: "GET",
      ...
      const sharedProjects = yield* SharedProjects;
      const manifest = yield* sharedProjects.getBootstrapManifest(
        user,
        projectId,
      );
...
export const sharedProjectsClaimInviteRouteLayer = HttpRouter.add(
  ...
  withSharedProjectErrorHandling(
    Effect.gen(function* () {
      ...
      const hosted =
        yield* hostedSharedProjectRequest<SharedProjectClaimResult>({
          user,
          method: "POST",
...
export const sharedProjectsDeleteRouteLayer = HttpRouter.add(
  ...
  withSharedProjectErrorHandling(
    Effect.gen(function* () {
      ...
      const hosted =
        yield* hostedSharedProjectRequest<DeleteSharedProjectResult>({
          user,
          method: "POST",
...
export const sharedProjectsImportThreadRouteLayer = HttpRouter.add(
  ...
  withSharedProjectErrorHandling(
    Effect.gen(function* () {
      ...
      if (hosted !== null) {
        const thread = hosted.threads.find(
        ...
        if (!thread.sessionSnapshot) {
          return yield* new SharedProjectsError({
            message:
              "This shared session does not have an importable snapshot yet.",
            status: 400,
          });
        }
        ...
        return HttpServerResponse.jsonUnsafe(
          result satisfies ImportSharedThreadResult,
          {
            status: 200,
            headers: browserApiCorsHeaders,
          },
...
export const sharedProjectsResolveThreadShareRouteLayer = HttpRouter.add(
  ...
  withSharedProjectErrorHandling(
    Effect.gen(function* () {
      ...
      const hosted =
        yield* hostedSharedProjectRequest<ResolvedSharedThreadShare>({
          user,
          method: "POST",
      ...
      const sharedProjects = yield* SharedProjects;
      const result = yield* sharedProjects.resolveSharedThreadShare(
        user,
        input.shareCode,
      );
      return HttpServerResponse.jsonUnsafe(result, {
...
export const sharedProjectsUpsertEnvironmentRouteLayer = HttpRouter.add(
  ...
  withSharedProjectErrorHandling(
    Effect.gen(function* () {
      ...
      const hosted =
        yield* hostedSharedProjectRequest<SharedProjectEnvironment>({
          user,
          method: "POST",
