...
validationLayer("CodexAdapterLive validation", (it) => {
  ...
  it.effect("maps codex model options before starting a session", () =>
    Effect.gen(function* () {
      ...
      NodeAssert.deepStrictEqual(validationRuntimeFactory.factory.mock.calls[0]?.[0], {
        ...
        stateDir: NodePath.join(process.cwd(), "userdata"),
        threadId: asThreadId("thread-1"),
        runtimeMode: "full-access",
      });
    }),
  ...
  it.effect("omits the standard codex service tier sentinel before starting a session", () =>
    Effect.gen(function* () {
      validationRuntimeFactory.factory.mockClear();
      const adapter = yield* CodexAdapter;
      ...
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-standard-tier"),
        modelSelection: createModelSelection(ProviderInstanceId.make("codex"), "gpt-5.3-codex", [
...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  ...
  it.effect("omits the standard codex service tier sentinel before sending a turn", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("sess-standard-tier"),
        runtimeMode: "full-access",
      });
      const runtime = sessionRuntimeFactory.lastRuntime;
      NodeAssert.ok(runtime);
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("maps Kami test harness completions to dynamic tool item events", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      const evidenceText =
        '{"runner":"playwright","status":"fail","success":false,"runId":"run-1","outputSummary":"Pairing screen was visible.","artifactPaths":{"trace":"C:/Users/THIS PC/.t3/dev/test-harness/run-1/trace.zip"}}\n';

      yield* runtime.emit({
        id: asEventId("evt-evidence-complete"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
      ...
      NodeAssert.equal(firstEvent.value.itemId, "call_1");
      NodeAssert.equal(firstEvent.value.payload.itemType, "dynamic_tool_call");
      NodeAssert.equal(firstEvent.value.payload.title, "Evidence run");
      NodeAssert.equal(firstEvent.value.payload.detail, "Pairing screen was visible.");
      NodeAssert.deepEqual(firstEvent.value.payload.data, {
        callId: "call_1",
        namespace: "kamicode",
        tool: "kami_test_harness",
        success: false,
  ...
  it.effect("surfaces project memory budget notices as runtime warnings", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const eventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      yield* runtime.emit({
        id: asEventId("evt-memory-warning"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-1"),
        createdAt: "2026-01-01T00:00:00.000Z",
        method: "kamicode/projectMemoryWarning",
        message: "Project memory is over 80% of its budget.",
      } satisfies ProviderEvent);
      ...
      NodeAssert.equal(event.value.payload.message, "Project memory is over 80% of its budget.");
    }),
  );
