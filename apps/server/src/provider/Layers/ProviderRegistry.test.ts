    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("persists the merged snapshot when a live update has empty models", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            const updateFiber = yield* registry.streamChanges.pipe(
              Stream.filter((providers) =>
                providers.some(
                  (provider) =>
                    provider.instanceId === cursorInstanceId &&
                    provider.checkedAt === refreshedProvider.checkedAt,
                ),
              ),
              Stream.runHead,
              Effect.forkChild,
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in KamiCode settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in KamiCode settings.");
        }),
      );
    });
