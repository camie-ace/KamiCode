...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("derives bypass permission mode from full-access runtime policy", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(createInput?.options.systemPrompt, {
        type: "preset",
        preset: "claude_code",
        append: appendRepositoryOperatingContract(
  ...
  it.effect("keeps the skill command block after image attachments", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(blocks[0]?.text ?? "", "<project_memory_policy>");
      assert.equal(blocks[1]?.type, "image");
      assert.deepEqual(blocks[2], { type: "text", text: "/review this screenshot" });
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("leaves a $ mention of an unknown or disabled skill as prose", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(
        promptText,
        applyProjectMemoryPromptPrefix({
          prompt: "run $deploy and echo $HOME",
          projectMemory: undefined,
        }),
      );
    }).pipe(
      ...
      const permissionResult = yield* Effect.promise(() =>
        canUseTool(
          ...
          },
          {
            ...
            requestId: "permission-harness-1",
          },
        ),
      );
