...
function dynamicToolTitle(payload: ProviderEvent["payload"]): string {
  const record = recordFromUnknown(payload);
  const namespace = stringFromUnknown(record?.namespace);
  const tool = stringFromUnknown(record?.tool);
  if (namespace === "kamicode" && tool === "kami_test_harness") {
    return "Evidence run";
  }
  return "Tool call";
}

function parseDynamicToolInputTextResult(
  payload: ProviderEvent["payload"],
): Record<string, unknown> | null {
  const record = recordFromUnknown(payload);
  const contentItems = Array.isArray(record?.contentItems) ? record.contentItems : [];
  for (const contentItem of contentItems) {
    const content = recordFromUnknown(contentItem);
...
function dynamicToolCompletionDetail(payload: ProviderEvent["payload"]): string | undefined {
  const result = parseDynamicToolInputTextResult(payload);
  const outputSummary = stringFromUnknown(result?.outputSummary);
  if (outputSummary) {
    return outputSummary;
  }
  const evidenceSummary = stringFromUnknown(result?.evidenceSummary);
  if (evidenceSummary) {
    return evidenceSummary;
  }
  const status = stringFromUnknown(result?.status);
  if (status) {
...
function mapToRuntimeEvents(
    ...
    return [
      {
        ...
        payload: {
          ...
          status: "inProgress",
          title: dynamicToolTitle(event.payload),
          ...(event.payload !== undefined ? { data: event.payload } : {}),
        },
      },
    ];
  ...
  if (event.method === "item/tool/call/completed") {
    const detail = dynamicToolCompletionDetail(event.payload);
    return [
      {
        ...runtimeEventBase(event, canonicalThreadId),
        type: "item.completed",
...
export const makeCodexAdapter = Effect.fn("makeCodexAdapter")(function* (
  ...
  const sendTurn: CodexAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    const codexAttachments = resolvedCodexAttachments.filter(
      (attachment): attachment is { readonly type: "localImage"; readonly path: string } =>
        attachment !== null,
    );
