  ...
  it.effect("sends selected project skills in Cursor's native slash form", () =>
    Effect.gen(function* () {
      ...
      const expectedPrompt = [
        ...
        { type: "text" as const, text: buildRuntimeInstructions({ harness: "Cursor" }) },
      ];
      ...
      assert.deepStrictEqual(
        ...
        [
          [
            {
              // Cursor excludes the synthetic runtime instruction from the
              // native transcript it returns, while retaining the repository
              // contract and project-memory-prefixed user prompt.
              prompt: expectedPrompt.slice(0, -1),
